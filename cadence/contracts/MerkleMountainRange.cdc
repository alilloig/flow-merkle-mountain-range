...
access(all) contract MerkleMountainRange {
    ...
    access(all) struct Proof {
        ...
        access(all) fun verify(data: [UInt8]): Bool {
            ...
            let mergePath: [UInt64] = MMRUtils.calcProofTreePathPositions(self.position, self.mmrSize)
            // Hash the data with the proof position to get the initial hash
            var nodeHash = MMRUtils.hashWithInteger(self.position, [data])
            log(nodeHash)
            ...
            for position in mergePath {
                if (!MMRUtils.isRightSibling(position)) {
                ...
                parentPosition = MMRUtils.getParentPosition(position)
                // The nodeHash on the last iteration will be the tree's peak hash
                nodeHash = MMRUtils.hashWithInteger(parentPosition, childHashes)
            ...
            calculatedRootHash = MMRUtils.hashWithInteger(self.getMMRSize(), peaksHashes)
    ...
    access(all) resource MMR {
        ...
        access(self) fun appendLeaf(leafData: [UInt8]) {
            ...
            var nodeHash = MMRUtils.hashWithInteger(nodePosition, [leafData])
            ...
            peaksPositions = MMRUtils.getPeaksPositions(self.getSize())
            // Update the peak hashes on the MMR state
            self.peaksHashes = MMRUtils.getHashesFromPositions(self.nodesHashes, peaksPositions)
            // Finally update the MMR root bagging all the new peaks along the new MMR size
            self.rootCommitment = MMRUtils.hashWithInteger(self.getSize(), self.peaksHashes)
        }
        ...
        access(all) fun generateProof(pos: UInt64): Proof {
            ...
            assert(MMRUtils.getHeight(pos) == 1, message: "Proofs can only be generated for leaf nodes")
            ...
            let proofPositions: MMRUtils.ProofPositions = MMRUtils.calcProofPositions(pos, size)
    ...
    init() {
        ...
        self.NO_ROOT = MMRUtils.hash(UInt8(0).toBigEndianBytes())
    }
